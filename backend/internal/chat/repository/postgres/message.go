...
func (r *MessageRepository) ListByChatID(ctx context.Context, chatID int64, cursor domain.Cursor) ([]*domain.Message, error) {
	...
	for _, row := range rows {
		messages = append(messages, &domain.Message{
			ID:         row.ID,
			ChatID:     row.ChatID,
			SenderType: domain.SenderType(row.SenderType),
			Content:    row.Content,
