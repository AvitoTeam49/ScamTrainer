...
type Service struct {
	...
	scenarios     domain.ScenarioProvider
	agent         agent.Agent
	historyLimit  int
	maxToolRounds int
}
...
func (s *Service) SendMessage(ctx context.Context, chatID, userID int64, content string) (*domain.Message, error) {
	...
	if !chat.IsActive() {
		return nil, domain.ErrChatFinished
	}
	...
	history, err := s.messages.ListByChatID(ctx, chat.ID, domain.Cursor{Limit: s.historyLimit})
	if err != nil {
		return nil, err
	}
...
func (s *Service) GetChat(ctx context.Context, chatID int64) (*domain.Chat, error) {
	return s.chats.GetByID(ctx, chatID)
}

func (s *Service) ListMessages(ctx context.Context, chatID int64, cursor domain.Cursor) ([]*domain.Message, error) {
...
func (s *Service) runAgent(ctx context.Context, chat *domain.Chat, messages []agent.Message) (string, error) {
	content := ""
...
func (s *Service) executeTool(ctx context.Context, chat *domain.Chat, call agent.ToolCall) (string, error) {
	switch call.Name {
...
func (s *Service) reportIncident(ctx context.Context, chat *domain.Chat, arguments []byte) (string, error) {
	var payload struct {
	...
	incidentType := domain.IncidentType(payload.Type)
	if !incidentType.Valid() {
		return toolFailure(domain.ErrInvalidIncidentKind), nil
	}
...
func (s *Service) finishChat(ctx context.Context, chat *domain.Chat, arguments []byte) (string, error) {
	var payload struct {
...
func buildMessages(prompt string, history []*domain.Message) []agent.Message {
...
func agentRole(sender domain.SenderType) agent.Role {
	if sender == domain.SenderTypeAgent {
